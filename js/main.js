...
function initYouTubeUploads() {
  ...
  try {
    // Fetch the JSON generated by GitHub Actions
    const response = await fetch('./yt-stats.json');
    if (!response.ok) throw new Error('Failed to load local stats JSON');

    const stats = await response.json();

    const formattedSubscribers = formatNumber(stats.subscriberCount);
    const formattedViews = formatNumber(stats.viewCount);
    const formattedVideos = formatNumber(stats.videoCount);
